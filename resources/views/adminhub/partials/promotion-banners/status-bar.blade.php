<div x-data="{ showStatusPicker: false }">
  <div class="relative">
    @if($promotionBanner->status == 'published')
    ...
    @endif
    @if($promotionBanner->status == 'draft')
    ...
    <ul x-cloak x-show="showStatusPicker" @click.outside="showStatusPicker = false" class="absolute right-0 z-10 -mt-24 overflow-hidden origin-bottom-right bg-white divide-y divide-gray-200 rounded-md shadow-lg bottom-12 w-72 ring-1 ring-black ring-opacity-5 focus:outline-none" tabindex="-1" role="listbox" aria-labelledby="listbox-label" aria-activedescendant="listbox-option-0">
      ...
      -->
      <li class="relative p-4 text-sm text-gray-900 cursor-default select-none @if($promotionBanner->status != 'published') hover:bg-gray-100 @endif" id="listbox-option-0" role="option">
        <button type="button" class="flex flex-col" wire:click.prevent="$set('product.status', 'published')">
          <div class="flex justify-between cursor-pointer">
            <!-- Selected: "font-semibold", Not Selected: "font-normal" -->
            <p class="@if($promotionBanner->status == 'published') font-semibold @else font-normal @endif">
              {{ __('adminhub::partials.products.status-bar.published.label')}}
            </p>
            <!--
      ...
      <li class="relative p-4 text-sm text-gray-900 cursor-default select-none @if($promotionBanner->status != 'draft') hover:bg-gray-100 @endif" id="listbox-option-0" role="option">
        <button type="button" class="flex flex-col cursor-pointer" wire:click.prevent="$set('product.status', 'draft')">
          <div class="flex justify-between ">
            <!-- Selected: "font-semibold", Not Selected: "font-normal" -->
            <p class="@if($promotionBanner->status == 'draft') font-semibold @else font-normal @endif">
              {{ __('adminhub::partials.products.status-bar.draft.label')}}
            </p>
            <!--
              Checkmark, only display for selected option.
