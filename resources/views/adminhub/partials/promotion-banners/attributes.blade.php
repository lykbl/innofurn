        ...
        <div class="grid grid-cols-2 gap-6 p-3 text-xs font-bold text-gray-500 uppercase bg-gray-50">
            <div class="flex justify-between">
                ...
                <x-hub::button type="button"
                               theme="gray"
                               size="xs"
                               wire:click="selectAll('{{ $group->id }}', '{{ $type }}')">
                    {{ __('adminhub::components.promotion-banners.index.select_all_btn') }}
                </x-hub::button>
            </div>
            ...
            <div class="flex justify-between">
                ...
                <x-hub::button type="button"
                               theme="gray"
                               size="xs"
                               wire:click="deselectAll('{{ $group->id }}', '{{ $type }}')">
                    {{ __('adminhub::components.promotion-banners.index.deselect_all_btn') }}
                </x-hub::button>
            </div>
        </div>
            ...
            <div class="py-3 space-y-2">
                ...
                @empty
                    <div class="text-sm text-gray-500">
                        {{ __('adminhub::catalogue.promotion-banners.attribute.no_attributes') }}
                    </div>
                @endforelse
            </div>
                        ...
                        <div>
                            @if (!$attribute->system)
                                <x-hub::button type="button"
                                               theme="gray"
                                               size="xs"
                                               wire:click="removeAttribute('{{ $attribute->id }}', '{{ $type }}')">
