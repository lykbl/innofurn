						...
						<x-slot name="content">
							<div class="grid grid-cols-4 gap-4 overflow-y-auto max-h-96">
								@forelse($chooseFrom as $productImage)
									<label
										@class([
												'cursor-pointer' => !\in_array($productImage->id, $this->currentImageIds, true),
												'opacity-50 cursor-not-allowed' => \in_array($productImage->id, $this->currentImageIds, true),
										])
									...
									>
										...
										<img
											src="{{ $productImage->getFullUrl('small') }}"
				...
				@foreach ($this->images as $key => $image)
					...
					>
						<div class="flex items-center w-full space-x-6">
							@if (count($images) > 1)
								<div
									class="text-gray-400 cursor-grab hover:text-gray-700"
									sort.handle
								>
									<x-hub::icon
										ref="selector"
							...
							<div class="w-full">
								<x-hub::input.text
									wire:model="images.{{ $key }}.caption"
									placeholder="Enter Alt. text"
								/>
							</div>

							<div class="flex items-center ml-4 space-x-4">
								<x-hub::tooltip text="Use as Hero image">
									<x-hub::input.toggle
								...
								@if (!empty($image['id']))
									<x-hub::tooltip :text="__('adminhub::partials.image-manager.remake_transforms')">
										<button
											wire:click.prevent="regenerateConversions('{{ $image['id'] }}')"
											href="{{ $image['original'] }}"
											type="button"
										>
											<x-hub::icon
								...
								<x-hub::tooltip :text="__('adminhub::partials.image-manager.delete_row_btn')">
									<button
										type="button"
